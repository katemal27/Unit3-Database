...
def add_movie():
    if request.method == 'POST':
        # process the form here later
        #request.form.get() - retrieves the vae from the form field
        #the paremter name must  match the 'name' attribute in the HTML form
        title = request.form.get('title')
        year = request.form.get('year',type=int) #converts str to int
        genre = request.form.get('director')
    ...
    #Comit to database (mkae changes perment)
    db.session.comit()
